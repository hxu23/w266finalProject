...
def assign_splits(imgs, params):
  ...
  for i,img in enumerate(imgs):
    if 'val2014' in img['file_path']:
      img['split'] = 'val'
      num_val = num_val + 1
    elif 'test2014' in img['file_path']:
      img['split'] = 'test'
      num_test = num_test + 1
    else:
      img['split'] = 'train'
...
def encode_captions(imgs, params, wtoi):
  ...
  category = np.zeros((N,max_length*2), dtype='uint32')
  full_category = np.zeros((N,max_length*6), dtype='uint32')
  bbox = np.zeros((N,max_length*6), dtype='uint64')
  caption_counter = 0
  counter = 1
  for i,img in enumerate(imgs):
    ...
    category[i, :len(img['categories'])] = img['categories']
    full_category[i, :len(img['full_categories'])] = img['full_categories']
    if max_length * 6 > len(img['bboxes']):
      bbox[i, :len(img['bboxes'])] = img['bboxes']
    else:
      bbox[i] = img['bboxes'][:max_length*6]
  ...
  return L, label_start_ix, label_end_ix, label_length, category, full_category, bbox
...
def main(params):
  ...
  train_val_imgs = []
  for img in imgs:
    if 'train2014/' in img['file_path'] or 'val2014/' in img['file_path']:
      train_val_imgs.append(img)
  # only want statistics from train and val set;
  # the test set contains dummy captions
  vocab = build_vocab(train_val_imgs, params)
  for img in imgs:
    img['final_captions'] = []
    for txt in img['processed_tokens']:
  ...
  L, label_start_ix, label_end_ix, label_length, category, full_category, bbox = encode_captions(imgs, params, wtoi)
  ...
  f.create_dataset("full_category", dtype='uint32', data=full_category)
  f.create_dataset("bbox", dtype='uint64', data=bbox)
